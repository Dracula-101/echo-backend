...
// LoadFromEnv resolves CONFIG_PATH and APP_ENV, loads the config, and
// runs validation so the rest of the program can rely on a fully checked
// Config. Single entry point main.go calls.
func LoadFromEnv() (*Config, error) {
	cfg, err := Load(
		env.GetEnv(envConfigPath, defaultConfigPath),
		env.GetEnv(envAppEnv, defaultEnvironment),
	)
	if err != nil {
		return nil, err
	}
