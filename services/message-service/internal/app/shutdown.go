...
func buildShutdown(deps shutdownDeps) *shutdown.Manager {
	...
	for _, ch := range deps.consumers {
		ch := ch // capture
		mgr.RegisterWithPriority(
			"kafka-consumer-"+ch.name,
			shutdown.Hook(func(ctx context.Context) error {
				deps.log.Info("Shutting down Kafka consumer",
					logger.String("name", ch.name),
					logger.String("topic", ch.topic),
				)
				ch.cancel()
				return ch.consumer.Close()
			}),
