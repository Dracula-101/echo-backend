...
func buildShutdown(deps shutdownDeps) *shutdown.Manager {
	...
	if deps.winStore != nil {
		mgr.RegisterWithPriority(
			"window-store",
			shutdown.Hook(func(ctx context.Context) error {
				deps.log.Info("Closing window store")
				if err := deps.winStore.Close(); err != nil {
					return err
				}
				return nil
			}),
	...
	if deps.memCache != nil {
		mgr.RegisterWithPriority(
			"mem-cache",
			shutdown.Hook(func(ctx context.Context) error {
				deps.log.Info("Closing in-memory cache")
				return deps.memCache.Close()
			}),
			shutdown.PriorityNormal,
		)
	}
