...
func (h *UserHandler) AcceptContact(handler *req.RequestHandler) {
	...
	if contact.Status != domain.ContactStatusPending {
		h.log.Info("Contact is not in pending status for accepting contact",
			logger.String("request_id", requestId),
			logger.String("correlation_id", correlationId),
			logger.String("contact_id", contactIdStr),
			logger.String("contact_status", string(contact.Status)),
		)
		response.BadRequestError(ctx, handler.Request(), handler.Writer(), "Contact is not in pending status", nil)
		return
	}
