...
CREATE INDEX idx_auth_sessions_user_device   ON auth.sessions(user_id, device_id) WHERE revoked_at IS NULL;
...
CREATE INDEX idx_auth_pwd_reset_token   ON auth.password_reset_tokens(token_hash);
...
CREATE INDEX idx_auth_email_verify_token    ON auth.email_verification_tokens(token_hash);
...
-- outbox
CREATE INDEX idx_auth_outbox_unpublished ON auth.outbox(created_at) WHERE published_at IS NULL;
CREATE INDEX idx_auth_outbox_user        ON auth.outbox(user_id);